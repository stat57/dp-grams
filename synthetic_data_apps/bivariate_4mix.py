...
def contour_plot_single(data, true_modes, ms_modes, dp_modes, results_dir):
    ...
    xgrid, ygrid = np.meshgrid(np.linspace(x_min, x_max, 200), np.linspace(y_min, y_max, 200))
...
def contour_plot_grid(data, true_modes, ms_modes_all, dp_modes_all, results_dir, nrows=4, ncols=5, figsize_per_subplot=(4.5,4.5)):
    ...
    x_min, x_max = data[:,0].min()-2, data[:,0].max()+2
    y_min, y_max = data[:,1].min()-2, data[:,1].max()+2
    xgrid, ygrid = np.meshgrid(np.linspace(x_min, x_max, 300), np.linspace(y_min, y_max, 300))
    positions = np.vstack([xgrid.ravel(), ygrid.ravel()])
    Z = np.reshape(kde(positions), xgrid.shape)
...
def mse_vs_n_samples(n_samples_list, epsilon, delta, p, n_runs, base_seed=42, results_dir="results/bivariate_4mix"):
    ...
    with open(csv_path, "w", newline="") as f:
        ...
        for i, n in enumerate(n_samples_list):
            writer.writerow([n, ms_mse_means[i], ms_mse_ses[i], dp_mse_means[i], dp_mse_ses[i], ms_time_means[i], ms_time_ses[i], dp_time_means[i], dp_time_ses[i]])
...
def privacy_utility_plot(n_samples_list, epsilon_values, ms_mse_n, ms_se_n, dp_mse_n, dp_se_n, dp_mse_eps_dict, results_dir):
    ensure_dir(results_dir)
    fig, ax = plt.subplots(figsize=(8,6))
    palette = sns.color_palette("Set2", len(n_samples_list))
    for i, n_samples in enumerate(n_samples_list):
        ms_mean, ms_se, dp_means, dp_ses = dp_mse_eps_dict[n_samples]
        ax.errorbar(epsilon_values, dp_means, yerr=dp_ses, marker='o', linestyle='-', linewidth=1.8, markersize=6, label=f"DP-GRAMS n={n_samples}", color=palette[i], capsize=3)
        ax.hlines(ms_mean, epsilon_values[0], epsilon_values[-1], colors=palette[i], linestyles='dashed', linewidth=1.2, label=f"MS n={n_samples}")
...
def main():
    ...
    ms_mse_n, ms_se_n, dp_mse_n, dp_se_n = mse_vs_n_samples(n_samples_list, epsilon, delta, p, n_runs, base_seed=base_seed, results_dir=results_dir)
    dp_mse_eps_dict = mse_vs_epsilon_for_n_samples(n_samples_list, epsilon_values, delta, p, n_runs, base_seed=base_seed, results_dir=results_dir)
