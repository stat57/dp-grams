# wine_compare_centroids_standardized_full_repro.py
import numpy as np
import os
import sys
import time
...
# Parameters
# ---------------------------
merge_n_clusters = 3  # Wine has 3 classes
ms_T = 7
ms_p = .1
eps_rr_high = 1e2
delta = 1e-5
...
base_rng_seed = 12
n_runs = 20
results_dir = "results/wine_centroid_comparison"
...
# ---------------------------
# 1) Mean-Shift + deterministic agglomerative merging
# ---------------------------
...
# ---------------------------
# PCA 2D visualization with one global legend
# ---------------------------
...
legend = fig.legend(global_handles, global_labels, fontsize=10, loc="lower center", ncol=6, bbox_to_anchor=(0.5, 0.01),
                    title="Cluster Assignments & Centroids")
plt.setp(legend.get_title(), fontsize=12, fontweight="bold")
fig.suptitle("Clustering Comparison on Wine Dataset", fontsize=20, y=0.90)

save_path_clusters = os.path.join(results_dir, "wine_clustering_comparison.png")
plt.tight_layout(rect=[0, 0.05, 1, 0.95])
plt.savefig(save_path_clusters, dpi=300, bbox_inches='tight')
plt.show()

# ---------------------------
# Privacy-Utility Tradeoff vs eps_modes (with runtime)
# ---------------------------
...
# Save privacy-utility CSV (with runtime)
...
print("All Wine experiments completed. Results saved in:", results_dir)
...
fig.suptitle("Privacy-Utility Tradeoff for Wine Dataset", fontsize=16, fontweight="bold", y=0.98)

save_path_privacy = os.path.join(results_dir, "wine_privacy_utility_standardized.png")
