# mnist_compare_centroids_pca_full_repro_corrected_complete.py
import numpy as np
import os
import sys
import time
...
p_seed = 0.1
base_rng_seed = 41
n_runs = 20   # reduced for speed on MNIST
...
print("MNIST data shape:", X_raw.shape)
...
if os.path.exists(pca_cache):
    ...
    pca_50.fit(X_raw)  # needed for inverse transform
else:
...
# ---------------------------
# Compute metrics and MSE (pixel space)
# ---------------------------
...
for alg, labels, modes in zip(algorithms, labels_list, modes_list):
    ...
    # Correct: inverse PCA -> scale to pixel space
...
for alg, rt in zip(algorithms, runtimes):
    vals = metrics[alg]
    print(f"{alg:<12} ARI={vals[0]:.3f}, NMI={vals[1]:.3f}, Silhouette={vals[2]:.3f}, MSE={mse_centroids[alg]:.6f}, Runtime={rt:.4f}s")
...
# ---------------------------
# Privacy-Utility Tradeoff vs epsilon_modes (with per-run timing)
# ---------------------------
...
print("All MNIST experiments completed. Results saved in:", results_dir)
