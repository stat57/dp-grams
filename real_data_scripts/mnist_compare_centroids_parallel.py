...
def main():
    ...
    with open(single_txt, "w") as f:
        for alg, rt in zip(algorithms, runtimes):
            vals = metrics[alg]
            f.write(f"{alg}: ARI={vals[0]:.4f}, NMI={vals[1]:.4f}, Silhouette={vals[2]:.4f}, MSE={mse_centroids[alg]:.6f}, Runtime={rt:.4f}s\n")
    ...
    legend = fig.legend(global_handles, global_labels, fontsize=10, loc="upper center", ncol=6, bbox_to_anchor=(0.5, 0.93), title="Cluster Assignments & Centroids")
