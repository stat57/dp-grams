# synthetic_data_apps/bivariate_5mix.py
import numpy as np
...
# -------------------------------
# Data generation: 5-mode bivariate t-mixture
# -------------------------------
...
# Grid plot
# -------------------------------
def contour_plot_grid(data, true_modes, ms_modes_all, dp_modes_all, results_dir, nrows=4, ncols=5, figsize_per_subplot=(4.5,4.5)):
    from scipy.stats import gaussian_kde
    ...
    Z = np.reshape(kde(np.vstack([xgrid.ravel(), ygrid.ravel()])), xgrid.shape)
    ...
    for slot in range(total_slots):
        ...
        ax.set_aspect('equal','box'); ax.set_title(f"Run {slot+1}", fontsize=10)

    legend_handles = [
        Line2D([0],[0], marker="*", color="#2ca02c", label="True modes", markeredgecolor="k", markersize=12, linestyle=""),
        Line2D([0],[0], marker="X", color="#1f77b4", label="Mean-Shift (MS)", markersize=10, linestyle=""),
        Line2D([0],[0], marker="o", color="#ff7f0e", label="DP-GRAMS", markeredgecolor="k", markersize=10, linestyle="")
    ]
    fig.legend(handles=legend_handles, loc="lower center", bbox_to_anchor=(0.5,0.04), ncol=3, frameon=True, fontsize=11)
    fig.suptitle("Mode Estimation Across 20 Runs: Bivariate 5-Modal t-Mixture", fontsize=14, y=0.94)
...
# -------------------------------
# MSE vs n samples (CSV)
# -------------------------------
def mse_vs_n_samples(n_samples_list, epsilon, delta, p, n_runs, base_seed=42, results_dir="results/bivariate_5mix"):
    ...
    for idx, n_samples in enumerate(n_samples_list):
        ...
        for run in range(n_runs):
            ...
            t0 = time.perf_counter()
            _, dp_raw = dp_grams(X=data, epsilon=epsilon, delta=delta, T=None, p0=p, rng=rng, h=h, hdp=0.3)
...
# MSE vs epsilon (CSV)
# -------------------------------
def mse_vs_epsilon_for_n_samples(n_samples_list, epsilon_values, delta, p,  n_runs, base_seed=42, results_dir="results/bivariate_5mix"):
    dp_mse_dict = {}
    ...
    for idx, n_samples in enumerate(n_samples_list):
        ...
        for eps_idx, eps in enumerate(epsilon_values):
            ...
            for run in range(n_runs):
                ...
                t0 = time.perf_counter()
                _, dp_raw = dp_grams(X=data, epsilon=eps, delta=delta, T=None, p0=p, rng=rng, h=h, hdp=0.3)
                dp_est = merge_modes(dp_raw)
                dp_time = time.perf_counter() - t0
...
# -------------------------------
# Privacy–Utility Plot (simplified: only MSE vs ε, legend inside)
# -------------------------------
...
def main():
    ...
    for run in range(n_runs):
        ...
        t0 = time.perf_counter()
        _, dp_raw = dp_grams(X=data, epsilon=epsilon, delta=delta, T=T, h=h, p0=p, rng=rng, hdp=0.3)
        dp_est = merge_modes(dp_raw, k=1)
    ...
    ms_mse_n, ms_se_n, dp_mse_n, dp_se_n = mse_vs_n_samples(n_samples_list, epsilon, delta, p,  n_runs=n_runs, base_seed=base_seed, results_dir=results_dir)
    dp_mse_eps_dict = mse_vs_epsilon_for_n_samples(n_samples_list, epsilon_values, delta, p, n_runs=n_runs, base_seed=base_seed, results_dir=results_dir)
