...
results_dir = "results/digits_centroid_comparison_wo_k"
...
# Parameters
# ---------------------------
merge_n_clusters = 10  # Digits has 10 classes
ms_T = int(np.log(n))
ms_p = 1
eps_rr_high = 1e2
delta = 1e-5
...
ms_merged_modes = merge_modes(raw_ms_modes, k=10)
...
fig.suptitle("Clustering Comparison on Digits Dataset", fontsize=20, y=0.98)

save_path_clusters = os.path.join(results_dir, "digits_clustering_comparison.png")
...
# ---------------------------
# Privacy-Utility Tradeoff vs eps_modes (with per-run timing)
# ---------------------------
...
for eps in eps_modes_list:
    ...
    for run in range(n_runs):
        ...
        dpms_times.append(time.time() - t0)
    ...
    for run in range(n_runs):
        ...
        dpkm_times.append(time.time() - t0)
...
fig.suptitle("Privacy-Utility Tradeoff for Digits Dataset", fontsize=16, fontweight="bold", y=0.98)

save_path_privacy = os.path.join(results_dir, "digits_privacy_utility_standardized.png")
